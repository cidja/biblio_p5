...
ob_start(); //Start of capture to put it in the variable at the end of the script 
    ...
    <div class="titleContainer">
        <h2>Bande dessinée en cours</h2>
    </div>
    <?php
    foreach($cartoonCurrent as $data) //source: https://www.php.net/manual/fr/control-structures.foreach.php
    {
        $title = "actuellement je lis : ". $data["title"];
        ?>
        
            <div>
                <a href="index.php?action=oneCartoon&amp;id=<?= $data["id"];?>">
                    <img class="imgCover +"src=<?=$data["cover"]; ?> alt="couverture de la bande dessinée <?= $data["title"]; ?>" title="couverture de la bande dessinée <?= $data["title"]; ?>" />
                </a>
            </div>
        <?php
    }
